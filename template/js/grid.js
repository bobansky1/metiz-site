...
(function(window, document, undefined) {
	...
	var GridProto;
	var showFirstRow; // показывать перный столбец
	var allblockwidth = 0;
	...
	GridProto.setOptions = function(options) {
		...
		this.options = {
			...
			fixedCols : 0, 
			selectedBgColor : "#EBF6FB", 
			fixedSelectedBgColor : "#ffad27", 
	...
	GridProto.convertDomDataToJsonData = function(data) {
		var sections = { "thead" : "Head", "tbody" : "Body", "tfoot" : "Foot" }, 
		    section, rows, row, cells, arr, arr2, arr_data, i, j, k, f, 
		    json = {};
		...
		if (((data || {}).tagName || "").toLowerCase() === "table") {
			for (i=0, j=0, rows=data.rows; row=rows[i]; i++) {
				...
				while (k) {
					...
					arr_data[f][k+1] = [ cells[k].getAttribute("data-weight"), cells[k].getAttribute("title")];
				}

			}
		}
		json['atribute'] = arr_data;
	...
	GridProto.convertData = function(data) {
		...
		if ((base = data.Head || data.Body || data.Foot || null)) {
			...
			if (data.Body) {
				this.convertDataItem(b, data.Body, "<DIV class='g_C g_BR g_R", cols, false, data.atribute);
			} else {
	...
	//////////////////////////////////////////////////////////////////////////////////
	GridProto.convertDataItem = function(arr, rows, rowClass, cols, allowColResize, atribute) {

		var rowIdx = rows.length, 
		    rowDiv, row, colIdx, attr, title;
		...
		while (rowIdx) {
			...
			while (colIdx) {
				--colIdx;
				if (colIdx==1 && row[colIdx]!=='' && row[colIdx]!='1') {
					showFirstRow = true;
		...
		while (rowIdx) {
			...
			while (colIdx) {
				...
				if (!!atribute) {
					attr = title = '';
					if (!!atribute[rowIdx][colIdx]) {
						attr = atribute[rowIdx][colIdx][0];
						title = atribute[rowIdx][colIdx][1];
					}
					arr[colIdx][rowIdx] = rowDiv + (row[colIdx]=='&nbsp;'? ' emptycell': ' fullcell') + "'" +  (row[colIdx]!='&nbsp;'? " data-weight='" +  (!!attr? attr: '')  +"' title='" +  (!!title? title: '')  +"'": "") + "'>" + (row[colIdx] || "&nbsp;");
				} else {
					arr[colIdx][rowIdx] = rowDiv + (row[colIdx]=='&nbsp;'? ' emptycell': ' fullcell') + "'>" + (row[colIdx] || "&nbsp;");					
				}
			}
		}
	...
	GridProto.generateGridSection = function(cols) {
		...
		while (colIdx) {
			if ((--colIdx) < fixedCols) {
				fHtml[colIdx] = "<DIV class='g_Cl g_Cl" + colIdx + " g_FCl' style='"+ (!showFirstRow? 'display:none': '') +"'>@" + colIdx + "@</DIV></DIV>";
				sHtml[colIdx] = "<DIV class='g_Cl g_Cl" + colIdx + " g_FCl' style='"+ (!showFirstRow? 'display:none': '') +"'></DIV>";
			} else {
	...
	GridProto.alignColumns = function(reAlign, fromInit) {
		...
		this.columnWidths = [];
		var minwidth = 0;
		var screenWidth = (document.getElementById('title_calc_text').offsetWidth) ? document.getElementById('title_calc_text').offsetWidth - (this.showFirstRow? 38: 160) : 1260;
		...
		for (var i=0, len=this.columns; i<len; i++) {
			...
			allblockwidth += colWidth;
		}
		
		if (allblockwidth < screenWidth)  {
			if (this.showFirstRow) {
				minwidth = Math.floor((screenWidth - 120) / (this.columns - 2));
				lastminwidth = Math.floor(screenWidth - 122 - (minwidth * (this.columns - 3)));
			}
			else minwidth = Math.floor((screenWidth - 55) / (this.columns - 3));
			console.log(this.columns);
		}
		
		//console.log(document.getElementById('title_calc_text').offsetWidth, screenWidth, minwidth)
		//console.log(this.showFirstRow)
		...
		for (var i=0, len=this.columns; i<len; i++) {
			...
			if (minwidth > 0 && i > 1) {
				colWidth = minwidth;
				if (len == i + 1 && lastminwidth > 0) colWidth = lastminwidth;
			}
			...
			if ((colBGColors[i] || "#ffffff") !== "#ffffff") {
				...
				rules[".g_Cl" + i]["color"] = "#fff";
				rules[".g_Cl" + i]["font-weight"] = "bold";
			}
			if (allowColumnResize) {
				rules[".g_RS" + i] = { "margin-left" : (!showFirstRow? 0 : colWidth - 2) + "px" };
			}
		}
		this.setRules();
		if (fromInit === true) {
